...
class _PinnedContextsRetriever:
    ...
    def search(self, question: str, *, top_k: int = 3) -> list[dict[str, Any]]:
        ...
        if callable(scoped_search):
            ...
            # Compatibility with simple retrievers; never fill unused slots
            # with another document just to reach top_k.
...
def answer(
    ...
    *,
    ...
    if selected_source_chunk_ids:
        service = _service_with_pinned_contexts(service, selected_source_chunk_ids)
    elif retrieved_contexts is not None and clarification_context is None:
