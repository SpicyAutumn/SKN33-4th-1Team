...
- 공유하기를 누르면 링크를 클립보드로 복사하고 `복사되었습니다` 안내가 2초 동안 표시됩니다. 주소 입력란은 표시하지 않으며, 복사 실패 시에는 짧은 실패 안내를 표시합니다. HTTP 접속으로 Clipboard API가 없거나 API 쓰기가 거절되면 임시 선택 영역을 사용하는 호환 복사를 시도합니다. 실제 복사 성공 여부를 확인하며 공유 링크 생성 오류는 별도로 표시합니다.
...
이 브랜치는 당시 커밋된 `main`에서 분리한 작업 폴더에 있습니다. 원래 작업 폴더의 미커밋 DB 정리 변경은 포함하지 않습니다. 최신 main의 마이페이지, 검색 문맥 유지, 관리자 검색 상세 조회·페이지 이동 및 정책 문서 모달 변경을 통합했습니다. 관리자 답변 조회는 읽기 전용으로 유지하며 공유 버튼을 제공하지 않습니다.
