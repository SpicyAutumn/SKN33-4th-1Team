...
| 열린 PR 조합 | #18 `2d82a1b` + 보완된 #23 `c80c327` + 후속 구현을 별도 로컬 작업 폴더에서 충돌 없이 병합. 프론트 테스트 16개, 제보·사진 검사, 빌드 통과. 원격 #18·#23은 변경하지 않음 |
...
2. `App.jsx`의 답변 카드 아래에 탐색 영역을 유지하면서 정정·요약·사진·출처·제보·게시판 흐름을 함께 확인합니다. 기존 답변 영역을 이전 버전으로 바꾸지 않습니다.
...
#24는 기반 main에 포함되어 있습니다. #23의 제보 패널 변경과 충돌하지 않도록 탐색은 답변 카드 바깥에 배치했고, `styles.css`는 수정하지 않았습니다. 위 조합 검사는 특정 커밋 기준이므로 후속 변경이 있으면 다시 확인합니다. #26의 README 변경은 이 기능의 선행 병합 조건이 아닙니다.

## 2026-09-22 답변 하단 배치 보완
근거 자료와 함께 알아보기를 답변 카드 하단의 좌우 영역으로 배치했습니다. 탐색 열기로 자료를 불러오면 중심·관계·여러 항목을 가지 형태로 표시합니다. 크게 보기는 전체 폭, 모바일은 세로 배치입니다. 기존 API·대상 선택·이전·재시도 동작을 유지합니다. 빌드 및 탐색 상태 5개 검사 통과. 이번 변경의 브라우저 시각 검증은 후속 확인 대상입니다.
