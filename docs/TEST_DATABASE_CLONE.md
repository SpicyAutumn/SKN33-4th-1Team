...
DB 삭제 옵션 `down --volumes`는 사용하지 않는다. preview 라벨을 제거해 앱이 중지되어도
DB·자격 증명·최종 배포 정보는 남는다. 다음 배포에서 같은 데이터를 사용한다.
...
현재 로컬 검증 결과는 원격 EC2 배포 성공을 뜻하지 않는다. 원격 적용에는 GitHub 배포 권한 또는
테스트 EC2에 접근할 AWS/SSH 인증이 필요하다.
