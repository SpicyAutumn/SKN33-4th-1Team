...
function AnswerView({ question, level, result, loading, onBack, onChangeLevel, onReport, onAsk }) {
  ...
  const mediaGroups = result?.media || [];
  return <section className="answer-page"><div className="answer-shell">
    ...
    {result && !result.error && <article className="ai-answer-card">
      ...
      <div className="ai-answer-body"><section className="core-summary"><b>★ 핵심 요약</b><p>{result.summary || result.message}</p></section><p className="full-answer">{result.message}</p>
        ...
        {mediaGroups.length > 0 && <section className="media-section" aria-labelledby="media-heading"><div className="media-heading"><div><span>공공누리 미디어</span><h2 id="media-heading">검색한 자료의 사진·지도·도표</h2></div><small>옆으로 넘겨 더 볼 수 있습니다.</small></div>{mediaGroups.map((group) => <section className="media-group" key={group.document_id}><h3>{group.article_title || citations.find((citation) => citation.document_id === group.document_id)?.title || "관련 자료"}</h3><div className="media-gallery">{group.images.map((image, index) => <figure className="media-card" key={`${group.document_id}-${image.mid || index}`}><img src={image.url} alt={image.title || image.description || group.article_title || "한국민족문화대백과사전 미디어"} loading="lazy" /><figcaption><div className="media-badges"><span>{image.role === "head" ? "대표" : "관련"}</span>{image.media_type && <span>{image.media_type}</span>}<strong>{image.kogl_label}</strong></div>{image.title && <b>{image.title}</b>}{image.description && <p>{image.description}</p>}<small>{image.attribution || "『한국민족문화대백과사전』"}</small>{image.copyright_display && <small>저작권 표시: {image.copyright_display}</small>}</figcaption></figure>)}</div></section>)}</section>}
