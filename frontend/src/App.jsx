...
export function AnswerView({ question, level, result, loading, loadingRecord, onBack, onChangeLevel, onSubmitReport, reportMode = "legacy", onAsk }) {
  ...
  return <section className={reportActive ? "answer-page report-active" : "answer-page"}><div className="answer-shell">
    ...
    {result && !result.error && !loading && result.response_type !== "needs_clarification" && <HeritageNetwork answerKey={result.request_id || result.search_record_id || question + level} question={question} citations={citations} />}
  </div></section>;
}
