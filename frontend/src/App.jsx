...
export default function App({ request = api } = {}) {
  ...
  const askQuestion = async (nextQuestion, nextLevel = level) => {
    ...
    setHistoryPage(false); setLoadingRecord(false);
  ...
  const openRecord = async (item) => {
    ...
    const version = ++requestVersion.current;
    setLoadingRecord(true); setHistoryPage(false); setQuestion(item.question); setLevel(item.audience_level); setLoading(true); setResult(null);
  ...
  const backToSearch = () => { ++requestVersion.current; setLoading(false); setHistoryPage(false); setResult(null); window.scrollTo({ top: 0, behavior: "smooth" }); };
  const logout = async () => { await request("auth/logout", { method: "POST" }); setUser(null); backToSearch(); };


  return <main>
    <header className="site-header"><a className="brand" href="#top" onClick={backToSearch}><span className="brand-mark" aria-hidden="true">📚</span><span>문화유산 AI 가이드</span></a>{user ? <div className="user">{showingAnswer && !historyPage && <details ref={historyMenu} className="history-menu" onKeyDown={(event) => { if (event.key === "Escape") { event.currentTarget.open = false; event.currentTarget.querySelector("summary").focus(); } }}><summary>내 검색 기록</summary><SearchHistory key={user.id} {...historyProps} /></details>}<b>{user.name}</b><button className="outline" onClick={logout}>로그아웃</button></div> : <button className="outline login-button" onClick={() => setAuthMode("login")}>로그인 / 회원가입</button>}</header>
