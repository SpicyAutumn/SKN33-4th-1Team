...
export default function ErrorReportBoard({ api, onBack, selectedId = null, onOpenDetail, onCloseDetail }) {
  ...
  if (selected) {
    ...
    return <section className="report-board-page"><div className="report-board-shell">
      ...
      {report && <article className="report-detail-card">
        <div className="report-detail-heading"><div><div className="report-categories">{reportCategories(report).map((category) => <span className="report-category" key={category}>{categoryLabel(category)}</span>)}</div><h1>{report.question}</h1></div><span className={`report-status status-${statusKey(report.status)}`}>{statusLabels[report.status] || report.status}</span></div>
        {report.selected_quotes?.length > 0 && <section><h2>선택한 답변 문구</h2><ul className="report-selected-quotes">{report.selected_quotes.map((quote) => <li key={quote.id ?? `${quote.start_offset}-${quote.end_offset}-${quote.text}`}>“{quote.text}”</li>)}</ul></section>}
  ...
  return <section className="report-board-page"><div className="report-board-shell">
    ...
    {state.loading ? <p className="board-state" role="status">오류 제보를 불러오고 있어요.</p> : state.error ? <div className="board-state board-error" role="alert"><p>{state.error}</p><button type="button" onClick={() => setRetry((value) => value + 1)}>다시 시도</button></div> : filteredItems.length ? <div className="report-list">{filteredItems.map((item) => <button type="button" className="report-list-item" key={item.id} onClick={() => openDetail(item)}><div><div className="report-categories">{reportCategories(item).map((category) => <span className="report-category" key={category}>{categoryLabel(category)}</span>)}</div><strong>{item.question_preview}</strong><small>{item.content_preview || "선택 문구 및 추가 설명 보기"}</small></div><div className="report-list-meta"><span className={`report-status status-${statusKey(item.status)}`}>{statusLabels[item.status] || item.status}</span><time dateTime={item.created_at}>{dateLabel(item.created_at)}</time><b aria-hidden="true">›</b></div></button>)}</div> : <div className="report-empty"><b>등록한 오류 제보가 없습니다.</b><p>답변에서 개선할 점을 발견하면 답변 오류 제보를 남겨 주세요.</p></div>}
  </div></section>;
}
