...
const reportCategories = (report) => (Array.isArray(report.categories) && report.categories.length ? report.categories : report.category ? [report.category] : []);
...
export default function AdminErrorReportBoard({ api, onBack }) {
  ...
  return <section className="admin-report-page"><div className="admin-report-shell">
    ...
    <div className="admin-workspace">
      <section className="admin-report-list" aria-label="오류 제보 목록">{state.loading ? <p className="admin-state" role="status">제보를 불러오고 있어요.</p> : state.error ? <div className="admin-state admin-error" role="alert"><p>{state.error}</p><button type="button" onClick={() => setRetry((value) => value + 1)}>다시 시도</button></div> : state.items.length ? state.items.map((item) => <button type="button" key={item.id} className={selectedId === item.id ? "selected" : ""} onClick={() => openDetail(item.id)}><span>{reportCategories(item).map(categoryLabel).join(" · ")}</span><strong>{item.question_preview}</strong><small>{item.reporter_name} · {dateLabel(item.created_at)}</small><em className={`admin-status status-${item.status}`}>{statusLabels[item.status] || item.status}</em></button>) : <p className="admin-state">처리할 제보가 없습니다.</p>}</section>
      <section className="admin-report-detail" aria-live="polite">{!selectedId ? <div className="admin-detail-empty"><b>왼쪽 목록에서 제보를 선택해 주세요.</b><p>제보 내용과 AI 답변을 확인한 뒤 처리 결과를 등록할 수 있습니다.</p></div> : detail.loading ? <p className="admin-state" role="status">제보 내용을 불러오고 있어요.</p> : detail.error ? <div className="admin-state admin-error" role="alert"><p>{detail.error}</p><button type="button" onClick={() => openDetail(selectedId)}>다시 시도</button></div> : report && <><header><div><div className="report-categories">{reportCategories(report).map((category) => <span className="report-category" key={category}>{categoryLabel(category)}</span>)}</div><h2>{report.question}</h2></div><span className={`admin-status status-${report.status}`}>{statusLabels[report.status] || report.status}</span></header><dl className="admin-reporter"><div><dt>제보자</dt><dd>{report.reporter_name}</dd></div><div><dt>이메일</dt><dd>{report.reporter_email}</dd></div><div><dt>등록일</dt><dd>{dateLabel(report.created_at)}</dd></div></dl><section><h3>제보 내용</h3><p className="admin-report-content">{report.content}</p></section>{report.selected_quotes?.length > 0 && <section><h3>선택한 답변 문구</h3><ul className="admin-selected-quotes">{report.selected_quotes.map((quote) => <li key={quote.id ?? `${quote.start_offset}-${quote.end_offset}-${quote.text}`}>“{quote.text}”</li>)}</ul></section>}<details className="admin-original-answer"><summary>제보 당시 AI 답변 전문 <span>펼치기</span></summary><p>{report.answer_text}</p></details><form onSubmit={save} className="admin-process-form"><h3>처리 등록</h3><label htmlFor="admin-status">처리 상태</label><select id="admin-status" value={status} disabled={saving} onChange={(event) => setStatus(event.target.value)}><option value="received">접수됨</option><option value="reviewing">확인 중</option><option value="completed">처리 완료</option></select><label htmlFor="admin-reply">담당자 답변</label><textarea id="admin-reply" value={reply} disabled={saving} maxLength="2000" placeholder="사용자에게 전달할 처리 결과를 입력해 주세요." onChange={(event) => setReply(event.target.value)} /><small>처리 완료 시 10자 이상의 담당자 답변이 필요합니다. {reply.length}/2000</small>{saveMessage && <p className={saveMessage === "처리 내용이 저장되었습니다." ? "admin-success" : "admin-save-error"} role="status">{saveMessage}</p>}<button className="primary" disabled={saving}>{saving ? "저장 중…" : "처리 내용 저장"}</button></form></>}</section>
    </div>
  </div></section>;
}
