...
import { reportPayload, reportTypes } from './reportPayload';
import './ErrorReportPanel.css';

export default function ErrorReportPanel({ recordId, answer, answerRef, mode = 'legacy', onSubmit, onOpenChange }) {
  ...
  const submit = async (event) => {
    ...
    setError('');
    try {
      const payload = reportPayload({ recordId, categories, content, quotes, answer, mode });
  ...
  return <>
    ...
    {open && <aside className={`report-dock${collapsed ? ' is-collapsed' : ''}`} aria-label="답변 오류 제보" onKeyDown={(event) => { if (event.key === 'Escape') close(); }}>
      ...
      {!collapsed && (success ? <p role="status">제보가 접수되었습니다.</p> : <form onSubmit={submit}>
        <p className="report-help">질문과 답변은 자동 연결됩니다. 답변 본문을 드래그한 뒤 선택 문구를 추가할 수 있어요.</p>
        <fieldset disabled={busy}><legend>오류 유형 {mode === 'v2' ? '(여러 개 선택 가능)' : '(1개 선택)'}</legend>{reportTypes.map(([value, label]) => <label key={value}><input type={mode === 'v2' ? 'checkbox' : 'radio'} name="report-category" checked={categories.includes(value)} onChange={(event) => setCategories(mode !== 'v2' ? [value] : event.target.checked ? [...categories, value] : categories.filter((item) => item !== value))} />{label}</label>)}</fieldset>
        <div className="report-quote-tools"><button type="button" disabled={busy || !candidate} onMouseDown={(event) => event.preventDefault()} onClick={addQuote}>{mode === 'v2' ? '선택 문구 추가' : '선택 문구를 제보 내용에 추가'}</button>{candidate && <p>선택: {candidate.text}</p>}</div>
        ...
        <label className="report-content-label" htmlFor="report-content">제보 내용</label><textarea id="report-content" disabled={busy} value={content} onChange={(event) => setContent(event.target.value)} aria-describedby="report-rule" />
        <p id="report-rule" className="report-help">{mode === 'v2' ? '선택 문구가 없거나 기타 유형이면 10자 이상 필요합니다.' : '선택 문구를 포함해 10자 이상 작성해 주세요.'} 최대 2,000자 · 현재 {Array.from(content.trim()).length}자</p>
