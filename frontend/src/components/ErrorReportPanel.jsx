  ...
  return <>
    ...
    {open && <aside className={`report-dock${collapsed ? ' is-collapsed' : ''}`} aria-label="답변 오류 제보" onKeyDown={(event) => { if (event.key === 'Escape') close(); }}>
      ...
      {!collapsed && (success ? <p role="status">제보가 접수되었습니다.</p> : <form onSubmit={submit}>
        ...
        <div className="report-quote-tools"><button type="button" disabled={busy || !candidate} onMouseDown={(event) => event.preventDefault()} onClick={addQuote}>{mode === 'v2' ? '선택 문구 추가' : capturePreview ? '선택 문구를 설명에 추가' : '선택 문구를 제보 내용에 추가'}</button>{candidate && <p>선택: {candidate.text}</p>}</div>
        ...
        <label className="report-content-label" htmlFor="report-content">{capturePreview ? '추가 설명' : '제보 내용'} {capturePreview && mode !== 'v2' ? (categories.includes('other') ? '(필수)' : '(선택)') : mode === 'v2' ? '(필수 조건은 아래 안내 참고)' : '(필수)'}</label><textarea id="report-content" aria-required={capturePreview && mode !== 'v2' ? categories.includes('other') : mode !== 'v2' || !quotes.length || categories.includes('other')} disabled={busy} value={content} onChange={(event) => setContent(event.target.value)} aria-describedby="report-rule" />
