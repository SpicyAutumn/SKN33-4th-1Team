  ...
  return <>
    ...
    {open && <aside className={`report-dock${collapsed ? ' is-collapsed' : ''}`} aria-label="답변 오류 제보" onKeyDown={(event) => { if (event.key === 'Escape') close(); }}>
      ...
      {!collapsed && (success ? <p role="status">제보가 접수되었습니다.</p> : <form onSubmit={submit}>
        ...
        <label className="report-content-label" htmlFor="report-content">추가 설명</label><textarea id="report-content" disabled={busy} value={content} onChange={(event) => setContent(event.target.value)} aria-describedby="report-rule" />
