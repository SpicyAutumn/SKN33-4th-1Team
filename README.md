...
공용 테스트 배포는 `main + preview 라벨 PR → 별도 테스트 EC2·MySQL` 흐름으로 운영 배포와 분리한다. 개인 결과 주소 `/search/…`는 계정 또는 검색한 브라우저의 쿠키로 접근을 확인하고, 사용자가 공유하기를 실행하면 `/share/…` 주소를 발급한다.
...
운영·데이터·실험·과거 기록을 포함한 전체 분류는 [문서 목차](docs/README.md)에서 확인한다.
